...
const Cart = require("../models/cartDataSchema");
 ...
 router.post("/",authenticate,
  async (req,res) => {
    ...
    try {
        ...
        prodId : req.body.prodId}).lean().exec();
...
router.patch("/:id",authenticate, async (req,res) => {
    ...
    try {
        const product = await Cart.findByIdAndUpdate(req.params.id,req.body,{new : true}).populate([{path : "productId",
        select : ["name","price","brand","image","count"]},{path:"userId",
        select :["name"]}]).lean().exec();
        res.send(product);    
    } catch (error) {
...
router.delete("/",authenticate, async (req,res) => {
    try {
        console.log("hello")
        const product = await Cart.deleteMany({"userId":req.userID});
        res.send(product);
    } catch (error) {
        res.send(error.message);
    }
